...
include_paths := /usr/local/cuda-11.2/include \
				 ...
				 /data-nbd/lxh/work2/download/TensorRT-8.0.1.6/include/

library_paths := /usr/local/cuda-11.2/lib64 \
				 /usr/local/lib \
				 /home/gaocy/anaconda3/envs/py38/lib \
				 /data-nbd/lxh/work2/download/TensorRT-8.0.1.6/lib
link_librarys := cudart opencv_core opencv_imgcodecs opencv_imgproc gomp \
				 pthread cublas python3.8 cudadevrt nvinfer nvcaffe_parser nvinfer_plugin nvonnxparser opencv_video opencv_tracking opencv_videoio opencv_highgui
...
# 定义cuda文件的编译方式
# workspace/%.cuo : src/%.cu
# 	@mkdir -p $(dir $@)
# 	@echo Compile $<
# 	@nvcc -rdc=true -shared $< -o $@ $(cu_compile_flags) -lcudadevrt
...
# $(workspace)/$(cubinary) : $(cu_objs)
# 	@mkdir -p $(dir $@)
# 	@echo Link $@
# 	@nvcc -dlink -o $@ $^ $(cu_compile_flags) $(culink_flags)
